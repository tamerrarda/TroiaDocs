...
The check is anchored to two values it takes from outside the report, so that the evidence can never nominate its own judge. The first is the operator's public key: the one signer the check will trust, supplied by the reviewer or read from the committed deployment record, never from the field the report names for itself. That is what stops a forged report from signing with an attacker's key and passing.

The second anchor exists because the first is not enough. The operator's key answers *who signed*, and never *where the money went*. An operator-signed payout into a look-alike pool — a contract the operator deployed themselves, holding none of the custodial USDC — would be perfectly self-consistent and would re-derive as a clean match, while the real pool never moved a unit. So the canonical pool contract is pinned from outside the report as well, and every payout must invoke that contract, checked on both sides: in the signed transaction and in the chain observation. Authorship and destination, each pinned to a record the report cannot touch.

Alongside them the report carries the network passphrase, needed to re-compute a transaction's true Stellar hash, and it is likewise read as plain data rather than taken from inside the signed blob it is meant to check. One field carried in the snapshot, the exchange rate that was applied, is deliberately left out of the comparison — its audit source is the accounting ledger, not the reconciler.
...
Both watchers have now run against the live chain and reconciled a real payout, finding it by the pool contract's own index. The accusing one has also been tested in the direction that actually matters: a payout was sent straight to the pool contract by hand, bypassing the backend, so its transaction never reached the authorised list — and the watcher flagged it as a rogue payout, unprompted, and recorded it permanently. See [Deployments](./deployments.md), which gives both runs transaction by transaction, and records what they still did *not* prove.
